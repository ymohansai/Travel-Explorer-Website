...
function showDetails(place) {
  ...
  const destination = document.getElementById("details");
  if (details[place]) {
    destination.innerHTML = `
      ${details[place].info}
      <div class="map-box">
        <iframe src="${details[place].map}" allowfullscreen="" loading="lazy"></iframe>
      </div>
    `;
  } else {
    destination.innerHTML = "<p>Details not available.</p>";
  }
}
...
function toggleMorePlaces() {
  ...
  if (!extraVisible) {
    ...
    // Apply current search filter to newly added cards
    filterDestinations();
