...
// Detailed info (unchanged content)
...
// Toggle View More / View Less (uses the exact image filenames & map links you provided)
function toggleMorePlaces() {
  const tourCards = document.getElementById("tourCards");
  const btn = document.getElementById("viewMoreBtn");

  if (!extraVisible) {
    // add extras (only once per click)
    const morePlaces = [
      {
    ...
    morePlaces.forEach(place => {
      ...
      card.className = "card extra-card"; // mark extras
      card.innerHTML = `
    ...
    // re-run filter so UI reflects current search input (if any)
    filterDestinations();
  }
}

// Filter function: checks all .card elements present in DOM (base + extras)
...
// Ensure click handlers still work if the file loads and user expects button binding
